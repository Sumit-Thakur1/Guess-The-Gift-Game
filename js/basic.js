//Script for level Basic
var counter=0;
var loose=0;
function  change(clicked_id){

if(clicked_id==value1|| clicked_id==value2|| clicked_id==value3){
document.getElementById(clicked_id).value="GIFT!!🎁";
document.getElementById(clicked_id).style.fontSize="18px";
document.getElementById(clicked_id).style.color="black";
document.getElementById(clicked_id).style.background="green";
counter++;
}
else if(clicked_id==bomb){
document.getElementById(clicked_id).value="Bomb💣!!";
document.getElementById(clicked_id).style.fontSize="18px";
document.getElementById(clicked_id).style.color="black";
document.getElementById(clicked_id).style.background="red";

document.getElementById("cont").innerHTML ="OPPs!!💣 you encountered a Bomb";
setTimeout('window.location.reload()', 2000);

}
else{
    
document.getElementById(clicked_id).value="MISS";
document.getElementById(clicked_id).style.fontSize="18px";
document.getElementById(clicked_id).style.color="yellow";
document.getElementById(clicked_id).style.background="black";
loose++;
}
if(counter==3){
    
    document.getElementById("cont").style.left="22%";
    document.getElementById("cont").innerHTML ="RESULT:CONGRATULATIONS!!🎉 You Took  "+(loose+counter)+" chances";
    setTimeout('window.location.reload()', 5000);
}
}


const rndfunc = function(less, large, avoid) {
    if(!avoid)
     avoid = []
     let rand = (less, large) => Math.floor(Math.random() * (large - less + 1)) + less;
        let retv = 0;
        while(avoid.indexOf(retv = rand(less,large)) > -1) { }
        return retv;
}


const  value1=Math.floor(Math.random() * 9)+1
let value2 = rndfunc(1, 9, [value1]);
let value3=rndfunc(1,9,[value1,value2])
let bomb=rndfunc(1,9,[value1,value2,value3])
console.log(value1);
console.log(value2);
console.log(value3);
console.log(bomb);



function home(){
        window.location.href="index.html";
}